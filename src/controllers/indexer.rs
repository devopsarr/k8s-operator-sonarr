...
pub async fn run(client: Client, context: Arc<Context>) {
    run_controller::<SonarrIndexer, _, _>(client, context, "SonarrIndexer", reconcile).await;
}
...
async fn reconcile_apply(indexer: Arc<SonarrIndexer>, ctx: Arc<Context>) -> Result<Action> {
    ...
    let config =
        get_sonarr_config(&ctx, client, &namespace, &indexer.spec.sonarr_instance_ref).await?;
    ...
    idx_resource.implementation = Some(Some(
        get_implementation_name(&indexer.spec.indexer_type).to_string(),
    ));
    ...
    let sonarr_indexer = if let Some(id) = indexer.status.as_ref().and_then(|s| s.id) {
        ...
        match indexer_api::update_indexer(&config, id, Some(false), Some(idx_resource.clone()))
            .await
        {
            Ok(i) => i,
            Err(_) => {
        ...
        if let Some(existing_idx) = existing
            .iter()
            .find(|i| i.name.as_ref().and_then(|n| n.as_ref()) == Some(&indexer.spec.name))
        {
            existing_idx.clone()
        } else {
            indexer_api::create_indexer(&config, Some(false), Some(idx_resource)).await?
    ...
    update_conditions(
        &mut conditions,
        ready_condition(true, "Synced", "Indexer synchronized with Sonarr"),
    );
...
pub async fn resolve_secret_value(
    ...
    if let Some(secret_ref) = secret_ref {
        ...
        let value_bytes = data
            .get(&secret_ref.key)
            .ok_or(Error::MissingApiCredentials)?;

        let value =
            String::from_utf8(value_bytes.0.clone()).map_err(|_| Error::MissingApiCredentials)?;
