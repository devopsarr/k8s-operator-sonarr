...
pub async fn run(client: Client, context: Arc<Context>) {
    run_controller::<SonarrImportList, _, _>(
        client,
        context,
        "SonarrImportList",
        reconcile,
...
async fn reconcile_apply(import_list: Arc<SonarrImportList>, ctx: Arc<Context>) -> Result<Action> {
    ...
    let namespace = import_list.namespace().ok_or(Error::MissingObjectKey(".metadata.namespace"))?;
    let name = import_list.name_any();

    info!("Reconciling SonarrImportList: {}/{}", namespace, name);

    let config = get_sonarr_config(&ctx, client, &namespace, &import_list.spec.sonarr_instance_ref).await?;

    // Build import list resource
    let mut resource = ImportListResource::new();
    resource.name = Some(Some(import_list.spec.name.clone()));
    resource.implementation = Some(Some(get_implementation_name(&import_list.spec.list_type).to_string()));
    ...
    resource.monitor_new_items = Some(convert_new_item_monitor_type(&import_list.spec.monitor_new_items));
    ...
    let sonarr_import_list = if let Some(id) = import_list.status.as_ref().and_then(|s| s.id) {
        resource.id = Some(id);
        match import_list_api::update_import_list(&config, id, Some(false), Some(resource.clone())).await {
            Ok(i) => i,
            Err(_) => {
                resource.id = None;
        ...
        if let Some(existing_item) = existing.iter().find(|i| i.name.as_ref().and_then(|n| n.as_ref()) == Some(&import_list.spec.name)) {
            existing_item.clone()
        } else {
            import_list_api::create_import_list(&config, Some(false), Some(resource)).await?
        }
    ...
    let mut conditions = import_list.status.as_ref().map(|s| s.conditions.clone()).unwrap_or_default();
    update_conditions(&mut conditions, ready_condition(true, "Synced", "Import list synchronized with Sonarr"));
...
async fn reconcile_cleanup(import_list: Arc<SonarrImportList>, ctx: Arc<Context>) -> Result<Action> {
    let client = &ctx.client;
    let namespace = import_list.namespace().ok_or(Error::MissingObjectKey(".metadata.namespace"))?;

    info!("Cleaning up SonarrImportList: {}/{}", namespace, import_list.name_any());

    if let Some(id) = import_list.status.as_ref().and_then(|s| s.id) {
        if let Ok(config) = get_sonarr_config(&ctx, client, &namespace, &import_list.spec.sonarr_instance_ref).await {
