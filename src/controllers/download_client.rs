...
async fn reconcile_apply(dc: Arc<SonarrDownloadClient>, ctx: Arc<Context>) -> Result<Action> {
    ...
    let namespace = dc.namespace().ok_or(Error::MissingObjectKey(".metadata.namespace"))?;
    ...
    dc_resource.implementation = Some(Some(get_implementation_name(&dc.spec.download_client_type).to_string()));
    ...
    let sonarr_dc = if let Some(id) = dc.status.as_ref().and_then(|s| s.id) {
        ...
        match download_client_api::update_download_client(&config, id, Some(false), Some(dc_resource.clone())).await {
            Ok(d) => d,
            Err(_) => {
                dc_resource.id = None;
                download_client_api::create_download_client(&config, Some(false), Some(dc_resource)).await?
            }
        }
    } else {
        let existing = download_client_api::list_download_client(&config).await?;
        if let Some(existing_dc) = existing.iter().find(|d| d.name.as_ref().and_then(|n| n.as_ref()) == Some(&dc.spec.name)) {
            existing_dc.clone()
        } else {
            download_client_api::create_download_client(&config, Some(false), Some(dc_resource)).await?
        }
    ...
    let mut conditions = dc.status.as_ref().map(|s| s.conditions.clone()).unwrap_or_default();
    update_conditions(&mut conditions, ready_condition(true, "Synced", "Download client synchronized with Sonarr"));
    ...
    clients_api.patch_status(&name, &PatchParams::default(), &Patch::Merge(&status_patch)).await?;
...
async fn reconcile_cleanup(dc: Arc<SonarrDownloadClient>, ctx: Arc<Context>) -> Result<Action> {
    ...
    info!("Cleaning up SonarrDownloadClient: {}/{}", namespace, dc.name_any());

    if let Some(id) = dc.status.as_ref().and_then(|s| s.id) {
        if let Ok(config) = get_sonarr_config(&ctx, client, &namespace, &dc.spec.sonarr_instance_ref).await {
