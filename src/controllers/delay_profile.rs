...
pub async fn run(client: Client, context: Arc<Context>) {
    run_controller::<SonarrDelayProfile, _, _>(
        client,
        context,
        "SonarrDelayProfile",
        reconcile,
...
async fn reconcile_apply(profile: Arc<SonarrDelayProfile>, ctx: Arc<Context>) -> Result<Action> {
    ...
    let namespace = profile.namespace().ok_or(Error::MissingObjectKey(".metadata.namespace"))?;
    let name = profile.name_any();

    info!("Reconciling SonarrDelayProfile: {}/{}", namespace, name);

    let config = get_sonarr_config(&ctx, client, &namespace, &profile.spec.sonarr_instance_ref).await?;
    ...
    resource.bypass_if_above_custom_format_score = Some(profile.spec.bypass_if_above_custom_format_score);
    ...
    let sonarr_profile = if let Some(id) = profile.status.as_ref().and_then(|s| s.id) {
        resource.id = Some(id);
        match delay_profile_api::update_delay_profile(&config, &id.to_string(), Some(resource.clone())).await {
            Ok(p) => p,
            Err(_) => {
                resource.id = None;
    ...
    let mut conditions = profile.status.as_ref().map(|s| s.conditions.clone()).unwrap_or_default();
    update_conditions(&mut conditions, ready_condition(true, "Synced", "Delay profile synchronized with Sonarr"));
...
async fn reconcile_cleanup(profile: Arc<SonarrDelayProfile>, ctx: Arc<Context>) -> Result<Action> {
    ...
    info!("Cleaning up SonarrDelayProfile: {}/{}", namespace, profile.name_any());

    if let Some(id) = profile.status.as_ref().and_then(|s| s.id) {
        if let Ok(config) = get_sonarr_config(&ctx, client, &namespace, &profile.spec.sonarr_instance_ref).await {
