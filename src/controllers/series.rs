...
pub async fn run(client: Client, context: Arc<Context>) {
    run_controller::<SonarrSeries, _, _>(
        client,
        context,
        "SonarrSeries",
        reconcile,
...
async fn reconcile_apply(series: Arc<SonarrSeries>, ctx: Arc<Context>) -> Result<Action> {
    ...
    let namespace = series.namespace().ok_or(Error::MissingObjectKey(".metadata.namespace"))?;
    let name = series.name_any();

    info!("Reconciling SonarrSeries: {}/{}", namespace, name);

    let config = get_sonarr_config(&ctx, client, &namespace, &series.spec.sonarr_instance_ref).await?;

    // Check if series exists by ID or by TVDB ID
    let existing_series = if let Some(id) = series.status.as_ref().and_then(|s| s.id) {
        match series_api::get_series_by_id(&config, id, Some(false)).await {
            Ok(s) => Some(s),
            Err(_) => None,
        }
    } else {
        // Try to find by TVDB ID
        let all_series = series_api::list_series(&config, Some(series.spec.tvdb_id), Some(true)).await?;
        all_series.into_iter().find(|s| s.tvdb_id == Some(series.spec.tvdb_id))
    };
    ...
    let sonarr_series = if let Some(mut existing) = existing_series {
        ...
        series_api::update_series(&config, existing.id.unwrap_or(0).to_string().as_str(), Some(false), Some(existing)).await?
    } else {
        // Need to lookup series from TVDB first
        let lookup_results = series_lookup_api::list_series_lookup(&config, Some(&format!("tvdb:{}", series.spec.tvdb_id))).await?;

        if lookup_results.is_empty() {
            return Err(Error::Other(format!("Series with TVDB ID {} not found", series.spec.tvdb_id)));
        }
        ...
        add_options.search_for_missing_episodes = Some(series.spec.add_options.search_for_missing_episodes);
        add_options.search_for_cutoff_unmet_episodes = Some(series.spec.add_options.search_for_cutoff_unmet_episodes);
    ...
    let mut conditions = series.status.as_ref().map(|s| s.conditions.clone()).unwrap_or_default();
    update_conditions(&mut conditions, ready_condition(true, "Synced", "Series synchronized with Sonarr"));
    ...
    let status = SonarrSeriesStatus {
        ...
        previous_airing: sonarr_series.previous_airing.flatten().map(|d| d.to_string()),
        network: sonarr_series.network.flatten(),
        series_status: sonarr_series.status.map(|s| format!("{:?}", s)),
    };

    let status_patch = serde_json::json!({ "status": status });
    series_api_k8s.patch_status(&name, &PatchParams::default(), &Patch::Merge(&status_patch)).await?;
...
async fn reconcile_cleanup(series: Arc<SonarrSeries>, ctx: Arc<Context>) -> Result<Action> {
    ...
    info!("Cleaning up SonarrSeries: {}/{}", namespace, series.name_any());

    if let Some(id) = series.status.as_ref().and_then(|s| s.id) {
        if let Ok(config) = get_sonarr_config(&ctx, client, &namespace, &series.spec.sonarr_instance_ref).await {
