...
use sonarr_operator::crds::quality_profile::{QualityGroup, Quality};
...
async fn test_quality_profile_full_lifecycle() {
    ...
    apply_resource(&ctx.client, &quality_profile).await.expect("Failed to create quality profile CR");

    // Wait for Ready condition
    tracing::info!("Waiting for quality profile to be ready...");
    let ready_qp = wait_for_ready::<SonarrQualityProfile>(
        &ctx.client,
        E2E_NAMESPACE,
        &qp_name,
        E2E_TIMEOUT,
    ...
    assert_eq!(sonarr_qp.name, profile_name, "Quality profile name mismatch");
    ...
    let updated_sonarr_qp = ctx.sonarr
    ...
    assert!(!updated_sonarr_qp.upgrade_allowed, "Upgrade should be disabled");
    ...
    let deleted = ctx.sonarr.find_quality_profile_by_name(&updated_profile_name).await;
    assert!(
